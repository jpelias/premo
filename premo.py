...
# compressed=True
def gen_eckey(passphrase=None, secret=None, pkey=None, compressed=True, rounds=1, version=0):
    k = KEY()
    if passphrase:
...
def test(otherversion):
    ...
    # by secret
    #print get_addr(gen_eckey(secret=("309F7C11F8083094FB1AEDFB91816ACE3D2581B686FFB2527A00BDD882040BD5").decode('hex')),version=otherversion)
    # by passphrase
    bitaddy = get_addr(gen_eckey(passphrase=premo),version=otherversion)
    addy = str(bitaddy[0])
    # print bitaddy[0]
    reto = "1AdU3EcimMFN7JLJtceSyrmFYE3gF5ZnGj"
    if reto == addy:
        print addy, premo,bitaddy[1]
    
    # by private key
    #print get_addr(gen_eckey(pkey='UYBFgra6FHs8ygyR7opXTzKdZQuMKiZdxYdWFos1i7WjZuNLivgs'),version=otherversion)
